...
export const SubmissionsInternalStatusTab = () => {
  ...
  const getJobSubmissionsinternalView = async (page = 1, pageSize = 10) => {
    ...
    try {
      ...
      if (response?.data?.masterFeeds) {
        setJobSubmissionsinternalView(response.data.masterFeeds);
        setTableParams({
          ...tableParams,
          pagination: {
  ...
  const handleTableChange = (pagination) => {
    ...
    getJobSubmissionsinternalView(pagination.current, pagination.pageSize);
  };

  const handleSortChange = (order) => {
    setSortOrder(order);

    if (!jobSubmissionsinternalView || jobSubmissionsinternalView.length === 0)
      return;

    const sortedData = [...jobSubmissionsinternalView].sort((a, b) => {
      if (order === "ascend") {
  ...
  return (
    <>
            ...
            <div className="d-flex justify-content-end align-items-center">
              <div className="toolbar ms-3 text-end ">
                <Button
                  type="primary"
                  size="small"
                  onClick={submissionsinternalAdd}
                >
                  Add
                </Button>
              </div>
      ...
      <div className="common-SpaceAdmin mt-4">
        <Table
          columns={submissionsinternalcolumns}
          dataSource={jobSubmissionsinternalView}
      ...
      <Modal
        open={openSubmissionsinternalAdd}
